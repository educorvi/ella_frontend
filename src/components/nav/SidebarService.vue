    ...
    <div>
        ...
        >
            <b-row align-v="center" class="clickable" @click="action">
                <b-col cols="1">
                    <div class="pl-2 sidebarSymbolParent">
                        <div
                            class="sidebarSymbol"
                            v-if="service.type === 'group'"
                            v-bind:class="{ open: cVisible }"
                ...
                <b-col>
                    <div
                        class="w-100"
                        v-bind:class="{ blend: isActive && !isLightColor }"
                    >
                        <h6 class="mb-0">{{ service.title }}</h6>
                        <p class="text-muted mb-0" v-if="service.description">
                            {{ service.description }}
                        </p>
                    </div>
                </b-col>
            ...
            >
                <div>
                    <side-bar-service
                        v-for="(subService, index) in service['services']"
                        :key="subService.title"
                        :service="subService"
                        :no-divider="index === service['services'].length - 1"
                    ></side-bar-service>
                </div>
            </b-collapse>
        </div>
...
<script lang="ts">
import { isLightColor } from '@/lib/global.js';
import { THEME_COLOR } from '@/config/ella.config.js';
    ...
    computed: {
        isActive() {
            return (
                ...
                this.$route.params.service === this.service.name
            );
        },
        isLightColor() {
    ...
    created() {
        this.cVisible =
            this.service['services']
                ?.map((s) => s.name)
                .includes(this.$route.params.service) || false;
    },
};
</script>
