<template>
    <div class="w-100">
        <b-button-group :vertical="stacked" class="mb-5 mt-2 w-100">
            <b-button
                v-for="(action, index) in service['formactions']"
                :key="action.name"
        ...
        <b-modal
            v-for="action in service['formactions']"
            :id="'modal_' + action.name"
            :key="'modal_' + action.name"
            hide-footer
            :title="
                action['modaltitle'] ||
                'Bitte zusätzliche Informationen angeben'
            "
        >
            <p v-if="action['modaltext']">{{ action['modaltext'] }}</p>
            <vue-json-form
                v-if="action.additional"
                :json="action.additional"
                :onSubmit="submitModal"
            >
                <hr />
                <b-button-group class="w-100">
                    <b-button @click="$bvModal.hide('modal_' + action.name)"
                        >Abbrechen</b-button
                    >
